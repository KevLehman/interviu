...
const candidateApproved = (questions = {}) => {
  ...
  // Hard logic to determine approval based on having more than a half questions ranked as average or more
  return performance.pass > Object.keys(questions).length / 2 ? "PASS" : "FAIL"
}

const mergeQuestionAndAnswers = (questions = [], answers = {}) => {
  return Object.keys(answers).reduce((accum, key) => {
              ...
              return (
                <div className="mb-2">
                  <h2 className="text-xl">
                    {mode} - {currentCandidateMode.tech} -{" "}
                    {candidateApproved(currentCandidateMode.answers)}
                  </h2>
                  <p>
                    Interviewer {currentCandidateMode.notes.interviewer} @{" "}
                  ...
                  {Object.keys(currentCandidateMode.answers).map(answer => (
                    <div className="mb-3">
                      <p className="text-lg">
                        {currentCandidateMode.answers[answer].question}
                      </p>
