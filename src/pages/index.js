...
const IndexPage = () => {
  ...
  return (
    ...
    <p className="text-xs text-gray-800 mb-8">We use the 3 base levels: Junior, Mid and Senior levels. Additionally, you can select 'ALL' that will show questions for all levels. This is useful if you want to interview a developer and you're not sure about its current level</p>
    <div className="w-full h-auto mb-6 flex items-baseline flex-row flex-wrap">
      {levels.map(sLevel => (
        <div className={`badge bg-red-200 ${level === sLevel ? 'bg-red-800 text-white' : 'text-red-800' }`} onClick={() => setLevel(sLevel)}>{sLevel}</div>
      ))}
    </div>
    <h3 className=" mt-8 text-xl">3. Optionally, enable Interview Mode</h3>
    <p className="text-xs text-gray-800 mb-8">Interview mode will take 10 random questions from the criteria you defined. For example, if you selected Node@Junior, the 10 questions will be for that level</p>
    <div className="w-full h-auto mb-6 flex items-baseline flex-row flex-wrap">
