...
export default ({ tech, level, isInterview = false }) => {
  ...
  let questions = level !== 'all' ? questionFile[level] : mergeQuestions(questionFile);
  ...
  return (
    <div className="w-full border-2 border-solid p-6 mt-10 mb-10">
      <n3 className="text-xl font-semibold text-header">Showing questions for <span className="uppercase">{tech}</span> designed for <span className="uppercase">{level}</span> devs</n3>
